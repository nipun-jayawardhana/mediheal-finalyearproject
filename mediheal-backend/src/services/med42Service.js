/**
 * Dedicated Med42 Biomedical Inference Service
 * Model: m42-health/Llama3-Med42-8B
 * Provider: Hugging Face Inference Router (featherless-ai)
 */

const MODEL_NAME = process.env.MED42_MODEL || process.env.HUGGINGFACE_MODEL || 'm42-health/Llama3-Med42-8B';
const ROUTER_ENDPOINT = process.env.MED42_ROUTER_ENDPOINT || 'https://router.huggingface.co/featherless-ai/v1/chat/completions';
...
/**
 * Perform inference call against Med42-8B via Hugging Face Router
 ...
 */
const analyzeSymptomsWithMed42 = async (input, param2 = '', param3 = 'mild', param4 = '') => {
  let clinicalCase;
  let reqId = '';
  ...
  const tag = reqId ? `[MED42][${reqId}]` : '[MED42]';
  const startedAt = Date.now();
  console.log(`${tag} Request preparation started`);

  const token = process.env.MED42_API_TOKEN || process.env.HUGGINGFACE_API_TOKEN;
  if (!token) {
    throw new Error('MED42_API_TOKEN or HUGGINGFACE_API_TOKEN is not configured in backend environment.');
  }
  ...
  const systemMessage = `You are Med42, a specialized clinical AI assistant for preliminary symptom analysis.
...
- Negative findings are explicitly ABSENT symptoms. Never treat negative findings as present symptoms.`;
...
Negative findings MUST be treated as absent.

JSON Output:`;

  console.log(`[MED42 PAYLOAD][${reqId || 'dev'}]\n\n${userMessage}`);
  ...
  try {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      if (globalController.signal.aborted || remainingTime <= SAFETY_MARGIN_MS) {
        ...
        lastError = lastError || new Error(`Med42 global deadline reached (${Math.min(TOTAL_AI_DEADLINE_MS, elapsed)}ms)`);
        break;
      }
      ...
      if (globalController.signal.aborted || remainingAfterDelay <= SAFETY_MARGIN_MS) {
        ...
        lastError = lastError || new Error(`Med42 global deadline reached (${Math.min(TOTAL_AI_DEADLINE_MS, Date.now() - startedAt)}ms)`);
        break;
      }
      ...
      try {
        ...
        if (response.status === 410) {
          console.warn(`${tag} HTTP 410 — model/endpoint deprecated or gone (non-retryable)`);
          lastError = new Error('Model/endpoint deprecated or gone (HTTP 410)');
          break;
        }
        ...
        if (!response.ok) {
          ...
          const permanentError = new Error(`Med42 API HTTP Error ${response.status}: ${errorText.substring(0, 200)}`);
        ...
        console.log(`${tag} Response received`);
        ...
        if (!rawContent) {
          ...
          throw new Error('Med42 returned an empty content response');
        }
        ...
        if (!parsedJSON) {
          ...
          throw new Error('Failed to parse structured JSON from Med42 response');
        }

        console.log(`${tag} JSON parsed successfully`);
        ...
        if (possibleConditions.length === 0) {
          ...
          throw new Error('Med42 did not produce valid possible conditions');
        }
        ...
        console.log(`${tag} Model: ${MODEL_NAME}`);
        console.log(`${tag} analysisSource: med42`);
        ...
        if (err.name === 'AbortError' || globalController.signal.aborted) {
          ...
          lastError = new Error(`Med42 request timed out after ${attemptMaxTimeout}ms`);
        } else {
          lastError = err;
  ...
  console.warn(`${tag} Primary inference failed: ${lastError ? lastError.message : 'Failed to complete Med42 inference'}`);
  throw lastError || new Error('Failed to complete Med42 inference');
};

module.exports = {
  analyzeSymptomsWithMed42,
  analyzeSymptomsWithOpenBioLLM: analyzeSymptomsWithMed42, // Backward-compatibility alias
  normalizeSpecialist,
  MODEL_NAME,
};
